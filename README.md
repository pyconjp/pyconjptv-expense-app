...
- フォームで明細・申請情報を入力し「申請を送信」を押すと、`claims/<申請ID>/` フォルダが作成され、添付ファイルと `claim.json`（申請データ）が保存されます。
- "履歴を表示する" にチェックすると、`claims/**/claim.json` を読み込んで履歴一覧を表示します。

## 保存仕様（JSONベース）

- 保存先: `claims/<申請ID>/claim.json`
- 申請ID: 送信時点の日時とランダムIDから生成
- `claim.json` の主なフィールド:
  - `claim_id`: 申請ID (例: `20251220103000-abc12345`)
  - `created_at`: 作成日時ISOフォーマット
  - `申請日`, `申請者名`, `タイトル`, `経費種別`, `合計金額`, `備考`
  - `経費項目リスト`: 明細の配列（各行に支払日・店名・金額・内容を保持するオブジェクト）
  - `attachments`: 添付ファイルの保存パス（文字列）の配列
  - `銀行名`, `支店名`, `口座種別`, `口座番号`, `口座名義`

## 追加でやること（今後）

- JIRA 連携
  - JIRAのAPIを使用して、PyCon JPのJIRA上に経費精算のチケットを作成する
  - タイトルは「経費精算：タイトル」とする
  - 本文は入力された情報を「フィールド名：値」の形でまずは箇条書きで保存する
  - 領収書などのファイルがあればチケットに添付する
  - JIRAと接続するためのユーザー名とAPIキーは設定ファイルまたは環境変数で設定可能にする
- 認証（Google OAuth 等）
  - アプリを利用するユーザーが誰であるかをGoogleのOAuth認証で記録できるようにする
  - ユーザーごとに「申請者情報」「振込先口座情報」をローカルに保持し、2回目以降のログイン時にデフォルト値として使用する
- スキーマバリデーションの強化（JSON Schema）
  - JSONに誤った値が入らないことをチェックする
  - 例：未来の日付、マイナスの金額、桁数の少ない口座番号など
  - Pydanticを使用する
...
|経費種別|経費の種類をあらかじめ用意された選択肢から選択。選択肢は「旅費交通費、消耗品費、交際費、雑費、その他」|プルダルンまたはラジオボタン|必須|
...
|備考|申請に関する補足事項や連絡事項を記入する|テキストエリア|任意|
